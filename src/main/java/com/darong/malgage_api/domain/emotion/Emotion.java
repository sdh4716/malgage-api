...
public class Emotion extends BaseTimeEntity {
    ...
    // ===== private 생성자 =====
    private Emotion(String name, Integer sortOrder, EmotionScope scope, User user) {
        validateName(name);
        validateScope(scope);
    ...
    /**
     * 기본 감정 생성 - 모든 사용자에게 제공되는 감정
     */
    public static Emotion createDefault(String name, Integer sortOrder) {
        return new Emotion(name, sortOrder, EmotionScope.DEFAULT, null);
    }
    ...
    public static Emotion createCustom(String name, User user, Integer sortOrder) {
        validateUser(user);
        Emotion emotion = new Emotion(name, sortOrder, EmotionScope.CUSTOM, user);
    ...
    public String toString() {
        return String.format("Emotion{id=%d, name='%s', scope=%s}",
                id, name, scope);
    }
}